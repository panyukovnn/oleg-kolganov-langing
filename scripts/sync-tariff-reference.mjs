...
if(args.includes('--check')){if(await fs.readFile(dest,'utf8')!==output)throw Error('Tariff reference is stale; regenerate and review.');console.log(`OK: ${actions.length} actions match canonical source and reviewed editorial changes.`);}else{await fs.writeFile(dest,output);console.log(`Exported ${actions.length} actions.`);}
